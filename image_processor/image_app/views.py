...
def cartoonize(request):
    ...
    if request.method == 'POST':
        ...
        if not imghdr.what(input_path):
            ...
            return JsonResponse(
                {"error": "Uploaded file is not a valid image"}, status=400)
        ...
        try:
            ...
            return FileResponse(
                ...
                as_attachment=True,
                filename=f"processed_{file.name}")
...
def style_transfer(request):
    ...
    if request.method == 'POST':
        ...
        if 'content_image' not in request.FILES or 'style_image' not in request.FILES:
            return JsonResponse(
                {"error": "Content and style images are required"}, status=400)
        ...
        content_path = os.path.join(
            UPLOAD_FOLDER, f"content_{content_file.name}")
        style_path = os.path.join(UPLOAD_FOLDER, f"style_{style_file.name}")
        output_path = os.path.join(
            STYLE_OUTPUT_DIR,
            f"styled_{content_file.name}")
        ...
        try:
            ...
            return JsonResponse(
                {"result_image": f"/media/style_results/styled_{content_file.name}"})
        except subprocess.CalledProcessError as e:
            return JsonResponse(
                {"error": "Failed to process style transfer", "details": str(e)}, status=500)
    else:
        return JsonResponse({"error": "Invalid request method"}, status=405)
